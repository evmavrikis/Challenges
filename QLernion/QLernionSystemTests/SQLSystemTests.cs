...
namespace QLernionSystemTests
{
    ...
    /// <summary>
    /// Basic systems tests for in physical sql mode.
    /// The service must have been started in the in memory mode (json settings config).
    /// This will not work for the azure service.
    /// </summary>
    [TestClass]
    public class SQLSystemTests
    {
        private const string LOCAL_URL = "http://localhost:1280/person";
        //private const string AZURE_URL = "http://localhost:1280/person";
        ...
        public async Task insertRetrieveSingle()
        {
            try
            {
                ...
                HttpResponseMessage response5 = await _client.GetAsync(_url + "/" + people.First().Id);
                ...
                Assert.IsTrue(people.Count == 1 && people.First().Id == p5.Id);
        ...
        [TestMethod]
        public async Task InsertModifyDelete()
        {
            try
            {
                ...
                person2.Salutation = "Hey Matey!";
                StringContent cont4 = new StringContent(JsonConvert.SerializeObject(people.ToArray()[1]), Encoding.UTF8, "application/json");
                HttpResponseMessage response4 = await _client.PutAsync(string.Format(_url + "/{0}", people.ToArray()[1].Id), cont4);

                HttpResponseMessage response5 = await _client.DeleteAsync(string.Format(_url + "/{0}", people.ToArray()[0].Id));
                response5.EnsureSuccessStatusCode();
                HttpResponseMessage response6 = await _client.DeleteAsync(string.Format(_url + "/{0}", people.ToArray()[1].Id));
                response6.EnsureSuccessStatusCode();

                HttpResponseMessage response7 = await _client.DeleteAsync(string.Format(_url + "/{0}", person2.Id));
                response7.EnsureSuccessStatusCode();

                HttpResponseMessage response8 = await _client.GetAsync(_url);
                string responseBody8 = await response8.Content.ReadAsStringAsync();

                var people8 = JsonConvert.DeserializeObject<List<Person>>(responseBody8);


                Assert.IsTrue(people8.Count == 0);
